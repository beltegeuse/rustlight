...
pub trait IntegratorGradient {
    fn compute_gradients(&mut self, scene: &Scene) -> Bitmap;
    fn iterations(&self) -> usize; //< Change into object and traits
}

impl Integrator for IntegratorGradient {
    fn compute(&mut self, scene: &Scene) -> Bitmap {
        let image = self.compute_gradients(scene);
        let image = gradient_reconstruct(scene, &image, self.iterations());
        image
    }
}

fn gradient_reconstruct(scene: &Scene, est: &Bitmap, iterations: usize) -> Bitmap {
    info!("Reconstruction...");
