...
impl PoissonReconstruction for UniformPoissonReconstruction {
    ...
    fn reconstruct(&self, scene: &Scene, est: &Bitmap) -> Bitmap {
        ...
        let mut image: Bitmap = Bitmap::new(Point2::new(0, 0), img_size.clone(), &vec![String::from("primal")]);
        for x in 0..img_size.x {
            for y in 0..img_size.y {
                let pos = Point2::new(x, y);
                let pix_value =
                    current.get(pos, &recons_name).clone() + est.get(pos, &very_direct_name).clone();
